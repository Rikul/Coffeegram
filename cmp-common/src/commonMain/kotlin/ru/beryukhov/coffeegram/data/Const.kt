 ...
 */
fun Map<CoffeeType, Int>.withEmpty(
    availableDrinks: List<CoffeeType> = CoffeeTypes.entries
): List<CoffeeTypeWithCount> {
    @Suppress("DataClassShouldBeImmutable")
    data class MutablePair(val ct: CoffeeType, var count: Int)

    val emptyList: MutableList<MutablePair> =
        availableDrinks.map { MutablePair(it, 0) }.toMutableList()
    this.forEach { entry: Map.Entry<CoffeeType, Int> ->
        emptyList.filter { it.ct.dbKey == entry.key.dbKey }.forEach { it.count = entry.value }
    }
